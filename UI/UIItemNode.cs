...
namespace DSPCalculator.UI
{
    public class UIItemNode
    {
        ...
        public UIItemNode(ItemNode node, UICalcWindow calcWindow) 
        {
            ...
            if (this.itemNode != null)
            {
                ...
                if (mainRecipeActive)
                {
                    ...
                    assemblerIconObj = GameObject.Instantiate(UICalcWindow.iconButtonObj);
                    ...
                    if (CalcDB.assemblerListByType.ContainsKey(type) && CalcDB.assemblerListByType[type].Count > 1)
                    {
                        ...
                        assemblerSelectionObj.transform.localPosition = new Vector3(170, 32, 0);
                        for (int i = 0; i < assemblers.Count; i++)
                        {
                            ...
                            aBtnObj.GetComponent<Image>().sprite = buttonBackgroundSprite;
                    ...
                    {
                        GameObject recipeChangeButtonObj = GameObject.Instantiate(UICalcWindow.iconButtonObj);
                        ...
                        recipeChangeButtonObj.GetComponent<Image>().sprite = biaxialArrowSprite;
                    ...
                    // 视为原矿按钮
                    GameObject treatAsOreButtonObj = GameObject.Instantiate(UICalcWindow.iconButtonObj);
                    ...
                    treatAsOreButtonObj.GetComponent<Image>().sprite = oreSprite;
                    ...
                    recipeGroupObj.transform.localPosition = new Vector3(-120, 20, 0);
                    RecipeProto recipeProto = itemNode.mainRecipe.recipeNorm.oriProto;
                    int posX = 0;
                    int posXDelta = 40;
                    for (int i = 0; i < recipeProto.Results.Length; i++)
                    {
                        GameObject recipeItem = GameObject.Instantiate(UICalcWindow.iconObj_NoTip, recipeGroupObj.transform);
                    ...
                    recipeArrow.GetComponent<RectTransform>().anchoredPosition3D = new Vector3(posX, -28, 0);
                    ...
                    // 切换增产按钮
                    incToggleObj = GameObject.Instantiate(UICalcWindow.incToggleObj, obj.transform);
                    ...
                    proliferatorSelectionObj.transform.localPosition = new Vector3(295, 32, 0);
                    //先创建一个禁用增产剂的图标
        ...
        public void RefreshAssemblerDisplay()
        {
            if(assemblerCountText != null && assemblerIconObj != null && itemNode.mainRecipe!=null)
            {
                ...
                if(assemblerItemId <= 0) // 代表全局和当前配方均无设置，那么读取最后一个配方
                {
        ...
        public void SetAssemblerPreference(int assemblerId)
        {
            if(itemNode != null && itemNode.mainRecipe != null)
            {
                ...
                recipeInfo.LoadConfig(preference);
                RefreshAssemblerDisplay();
            }
